...
- **MODIFIED (breaking convention)**: Path-mode routing now requires `WebRouting:Path` to equal the blueprint's short name (case-insensitively). The route index resolves a path-segment request via `IShellBlueprintProvider.GetAsync(segment)` to preserve `007`'s O(1)-per-tenant scaling for the multi-tenant common case; arbitrary `Path != Name` configurations (which the legacy resolver supported via per-request enumeration) are no longer routable. `ShellRouteIndexBuilder` logs a `Warning` at index-population time when the convention is violated. Existing deployments where `WebRouting:Path == Name` (the convention everywhere in our codebase and in `Elsa.ModularServer.Web`) need no change. Deployments relying on `Path != Name` must rename the blueprint, route via `Host`/`HeaderName`/`ClaimKey`, or pin to the previous version.
